...
export default function UpdatePost() {
  ...
  const handleSubmit = async (e)=>{
    ...
    try {
      ...
      const res = await fetch(`/api/post/update/${params.postId}/${currentUser._id}`,{
        method:"POST",
        headers:{
      ...
      setPublishError(null)
      iziToast.success({
        message: '<b>Post updated successfully!</b>',
        position: 'topRight',
        timeout:1500
      });
    ...
    <div className='p-3 max-w-3xl mx-auto min-h-screen'>
      ...
      <form className='flex flex-col gap-4' onSubmit={handleSubmit}>
        <div className='flex flex-col gap-4 sm:flex-row justify-between'>
          <TextInput type='text' placeholder='Title' required className='flex-1' value={formData.title} onChange={(e)=>setFormData({...formData,title:e.target.value})}/>
        ...
        <ReactQuill theme='snow' value={formData.content} className='h-72 mb-12' id='content' onChange={(value)=>setFormData({...formData,content:value})}/>
